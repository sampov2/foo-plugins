...
% Section I, mid-c, 8'
Figure \ref{fig:comp_manual_i} shows C2 played on section I with only the 8' draw bar engaged. The line depicts the frequency spectrum of the emulation output. The crosses show peaks measured from a YC-20 organ using the same settings.
...
Figure \ref{fig:comp_manual_ii} shows C2 played on a fully bright section II with only the 8' draw bar engaged. This measurement was done at the same overall level as measurements in figure \ref{fig:comp_manual_i}. There is a slight overall volume difference, but the measurement also shows the section II filters are not perfect. However, while C2 harmonics do not line up perfectly, the balance between harmonics of some other notes (for example C3) match perfectly to the real organ.

%This shows that there is an overall volume difference between the two sections, but the balance between harmonics of the sound is well emulated.
...
\caption{Faust SVG output of section II high pass filtering for a single draw bar. The parameters for the filters are resistance in ohms and capacitance in microfarads.}
