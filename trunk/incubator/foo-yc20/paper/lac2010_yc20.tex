...
Sometimes work halted as the emulation started behaving unexpectedly. Debugging the issue revealed that Faust has no problem with using the same variable name twice. For instance a function definition with a \emph{with} clause can contain definitions under the same symbol name as parameters to the original functions.
...
The above Faust code is ambiguous, but it is compiled without any warning. This can lead to anywhere from unexpected results to complete failure. It would be good form to at least warn the user of the situation.
